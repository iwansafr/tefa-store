...
if(!empty($cat_id))
{
  $cat = $this->data_model->get_one_data('content_cat', ' WHERE id = '.$cat_id);
...
if(!empty($last_id))
{
  if(!empty($_POST))
  {
    ...
    $this->data_model->set_data('content_cat', $last_id, $post);
  }
}
...
if(!empty($get_id))
{
  if(!empty($_POST))
  {
    $post       = array();
    $uniqe_id   = '';
    $check_slug = $this->data_model->get_one('content_cat', 'slug', " WHERE id = {$get_id} AND slug = '".$this->input->post('slug')."'");
    if(empty($check_slug))
    {
      $check_slug = slug($this->input->post('title'));
      $check_slug = $this->data_model->get_one('content_cat', 'slug', " WHERE slug = '".$check_slug."'");
      if(empty($check_slug))
      {
        $check_slug = slug($this->input->post('title'));
      }else{
    ...
    $slug         = implode('-', $array_slug);
    $post['slug'] = slug($slug);
    $this->data_model->set_data('content_cat', $get_id, $post);
  }
}